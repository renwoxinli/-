...
public class SettingsActivity____ extends PreferenceActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		chooseGooglePlayMusicAccountPreference = preferenceManager
				.findPreference("preference_key_google_play_music_account");
		...
		crossfadeTracksDurationPreference = preferenceManager
				.findPreference("preference_key_crossfade_tracks_duration");
		...
		if (sharedPreferences.getBoolean("TRIAL", true) == false) {
			...
			PreferenceCategory upgradePrefCategory = (PreferenceCategory) preferenceManager
					.findPreference("upgrade_pref_category");
		...
		trackChangeAnimationPreference.setOnPreferenceClickListener(new OnPreferenceClickListener() {
			...
			public boolean onPreferenceClick(Preference preference) {
				...
				View layoutView = mSettingsActivity.getLayoutInflater()
						.inflate(R.layout.generic_message_listview_dialog_layout, null);
			...
			public boolean onPreferenceClick(Preference preference) {
				...
				builder.setSingleChoiceItems(R.array.enable_disable, currentPreference, new OnClickListener() {
					...
					public void onClick(DialogInterface dialog, int which) {
						...
						// Enable/disable the lockscreen controls for this
						// session.
						try {
							if (which == 0) {
								Toast.makeText(mContext, R.string.changes_saved, Toast.LENGTH_SHORT).show();
								...
								if (sharedPreferences.getBoolean("SERVICE_RUNNING", false) == true) {
									Toast.makeText(mContext, R.string.lockscreen_controls_disabled_next_run,
		...
		chooseGooglePlayMusicAccountPreference.setOnPreferenceClickListener(new OnPreferenceClickListener() {
			...
			public boolean onPreferenceClick(Preference arg0) {
				...
				// Create a new array with a list of all account names on the
				// device.
				for (int i = 0; i < accounts.length; i++) {
					accountNames[i + 1] = accounts[i].name;
				}

				// Get the current account name (if it exists) and get the index
				// of the account name.
				...
				// Display a list of all accounts that are currently on the
				// device.
				AlertDialog.Builder builder = new AlertDialog.Builder(mContext);
				...
				builder.setSingleChoiceItems(accountNames, accountIndex, new OnClickListener() {
					...
					public void onClick(DialogInterface dialog, int position) {
						if (position != 0) {
							...
							sharedPreferences.edit().putString("GOOGLE_PLAY_MUSIC_ACCOUNT", accountNames[position])
									.commit();
							AsyncGoogleMusicAuthenticationTask task = new AsyncGoogleMusicAuthenticationTask(
									mContext.getApplicationContext(), mSettingsActivity, false, accountNames[position]);
		...
		rescanFoldersPreference.setOnPreferenceClickListener(new OnPreferenceClickListener() {
			...
			public boolean onPreferenceClick(Preference arg0) {
				...
				// Seting the "REBUILD_LIBRARY" flag to true will force
				// MainActivity to rescan the folders.
		...
		blacklistManagerPreference.setOnPreferenceClickListener(new OnPreferenceClickListener() {
			...
			public boolean onPreferenceClick(Preference arg0) {
				 ...
				 * PreferenceDialogLauncherActivity.class);
				 * intent.putExtra("INDEX", 8); startActivity(intent);
				 */

				/*
				 * String[] blacklistManagerChoices = {
				 * mContext.getResources().getString(R.string.
				 * manage_blacklisted_artists),
				 * mContext.getResources().getString(R.string.
				 * manage_blacklisted_albums),
				 * mContext.getResources().getString(R.string.
				 * manage_blacklisted_songs),
				 * mContext.getResources().getString(R.string.
				 * manage_blacklisted_playlists) };
				 * 
				 ...
				 * builder.setItems(blacklistManagerChoices, new
				 * OnClickListener() {
				 * 
				 * @Override public void onClick(DialogInterface dialog, int
				 * which) { Bundle bundle = new Bundle();
				 * 
				 * if (which==0) { bundle.putString("MANAGER_TYPE", "ARTISTS");
				 * } else if (which==1) { bundle.putString("MANAGER_TYPE",
				 * "ALBUMS"); } else if (which==2) {
				 * bundle.putString("MANAGER_TYPE", "SONGS"); } else if
				 * (which==3) { bundle.putString("MANAGER_TYPE", "PLAYLISTS"); }
				 * 
				 * dialog.dismiss(); Intent intent = new Intent(mContext,
				 * BlacklistManagerActivity.class); intent.putExtras(bundle);
				 * startActivity(intent);
				 * 
				 * }
				 * 
		...
		/*
		 * bluetoothControlsPreference.setOnPreferenceClickListener(new
		 ...
		 * });
		 * 
		 * headphonesUnplugActionPreference.setOnPreferenceClickListener(new
		 * OnPreferenceClickListener() {
		 * 
		 * @Override public boolean onPreferenceClick(Preference arg0) { // TODO
		 * Auto-generated method stub return false; }
				...
				builder.setSingleChoiceItems(enableDisableArray, currentSelection, new OnClickListener() {
					...
					public void onClick(DialogInterface dialog, int which) {
						...
						if (which == 0) {
							...
							if (mApp.isServiceRunning() && mApp.getService().getHandler() != null) {
								mApp.getService().getHandler()
										.removeCallbacks(mApp.getService().startCrossFadeRunnable);
		...
		crossfadeTracksDurationPreference.setOnPreferenceClickListener(new OnPreferenceClickListener() {
			...
			public boolean onPreferenceClick(Preference preference) {
				...
				RelativeLayout dialogView = (RelativeLayout) mSettingsActivity.getLayoutInflater()
						.inflate(R.layout.dialog_crossfade_duration, null);
		...
		contactUsPreference.setOnPreferenceClickListener(new OnPreferenceClickListener() {
			...
			public boolean onPreferenceClick(Preference arg0) {
				...
				Intent emailIntent = new Intent(Intent.ACTION_SENDTO,
						Uri.fromParts("mailto", "jamsmusicplayer@gmail.com", null));
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		// Result Code 45 = UserRecoverableAuthenticationException from
		// GooglePlayMusicAuthenticationDialog.
		if (requestCode == 45) {
	 ...
	 * 
	 * String base64EncodedPublicKey = ""; base64EncodedPublicKey = Common.uid4
	 * + Common.uid2 + Common.uid6 + Common.uid1 + Common.uid3 + Common.uid5;
	 * 
	 * mHelper = new IabHelper(this, base64EncodedPublicKey);
	 * mHelper.startSetup(new IabHelper.OnIabSetupFinishedListener() {
	 * 
	 * @Override public void onIabSetupFinished(IabResult result) { if
	 * (!result.isSuccess()) { //In-app billing could not be initialized.
	 * Toast.makeText(mSettingsActivity, R.string.unable_to_reach_google_play,
	 * Toast.LENGTH_LONG).show(); } else { //In-app billing was initialized
	 * successfully. try { mHelper.launchPurchaseFlow(mSettingsActivity,
	 * ITEM_SKU, 10001, mPurchaseFinishedListener, ""); } catch (Exception e) {
	 * e.printStackTrace(); Toast.makeText(mContext,
	 * R.string.unable_to_reach_google_play, Toast.LENGTH_LONG).show();
	 * finish(); } }
	 * 
	 * }
	 * 
	...
	IabHelper.OnIabPurchaseFinishedListener mPurchaseFinishedListener = new IabHelper.OnIabPurchaseFinishedListener() {
		...
		public void onIabPurchaseFinished(IabResult result, Purchase purchase) {
			...
			if (result.isFailure()) {
				...
				Toast.makeText(mContext, R.string.Xuri_trial_time_removed, Toast.LENGTH_LONG).show();
